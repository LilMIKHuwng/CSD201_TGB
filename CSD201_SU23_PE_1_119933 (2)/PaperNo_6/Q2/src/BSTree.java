...
public class BSTree {
    ...
    */
    ...
    public void breadthCondition(Node p, RandomAccessFile f) throws Exception {
        if(p == null) 
            return;
        Queue q = new Queue();
        q.enqueue(p);
    ...
    void f3() throws Exception {
        ...
        Bird newBird = new Bird("V", 100-k, 2*k);
    ...
    void resetLeaf(Node node){
        if (node == null){
            return;
        } 
        
        if (node.left == null && node.right == null){
            node.getInfo().setColor(0);
        }
        
        resetLeaf(node.left);
        resetLeaf(node.right);
    }
